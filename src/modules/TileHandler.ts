...
export class FarmTileHandler implements TileHandler {
  updateState(space: BoardSpace, gameManager: GameManager): SpaceUpdate | null {
    ...
    const thrivingCondition = peopleCount > 1 && peopleCount <= 4 && treeCount <= 3;

    // Farms struggle without people or too many trees
    const strugglingCondition = peopleCount === 0 || peopleFarmDiff >= 5 || treeCount >= 5;
...
export class PowerTileHandler implements TileHandler {
  ...
  updateState(space: BoardSpace, gameManager: GameManager): SpaceUpdate | null {
    ...
    const thrivingCondition =
      peopleCount >= minPeople && peopleCount <= maxPeople && powerCount <= maxPower;
...
function handleTileState(
  space: BoardSpace,
  thrivingCondition: boolean,
  strugglingCondition: boolean
): SpaceUpdate | null {
  const tile = space.tile;
